...
├── README.md
├── database/
│ ├── portfolio.db # SQLiteデータベース（再生成済み）
│ └── insert_sample_data.sql # データ挿入スクリプト
├── queries/ # SQLクエリファイル（Q1〜Q5）
├── images/ # 実行結果CSV（q1_result.csv など）
...
- SQLiteの `DATE('now', '-30 days')` を用いて動的な日付フィルタを実装  
- サブクエリとJOINを併用し、現場でも応用可能な形式に設計  
- TablePlus等で結果を検証し、実行結果CSVで出力確認済み

---

## ✅ 使用方法

1. SQLite対応ツール（例：DB Browser for SQLite, TablePlus）で `database/portfolio.db` を開く  
2. `queries/` フォルダの `.sql` をコピーしてクエリ実行  
3. `images/` 内のCSVファイルと結果を照らし合わせて確認  
